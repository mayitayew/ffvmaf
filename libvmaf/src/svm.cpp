...
class SVMModelParserFileSource {
 ...
 public:
  ...
  bool get(TValue& value, char delim = '\n') {
    if (buffer >> value) {
  ...
  template <typename TValue>
  bool get_array(TValue* value, size_t len) {
...
class SVMModelParserBufferSource {
 ...
 public:
  SVMModelParserBufferSource(const char* buffer, size_t len)
      : buffer(std::string(buffer, len)) {}
...
class SVMModelParser {
 ...
 public:
  SVMModelParser(TSource&& model_source)
      : model_source(std::move(model_source)) {}

  bool parse() {
    model = Malloc(svm_model, 1);
    memset(model, 0, sizeof(struct svm_model));
    const bool parsed_header = parse_header();
    if (!parsed_header) {
      fprintf(stderr, "Parsing header failed\n");
      svm_free_and_destroy_model(&model);
      return false;
    }
    const bool parsed_support_vectors = parse_support_vectors();
    if (!parsed_support_vectors) {
      fprintf(stderr, "Parsing support vectors failed\n");
      svm_free_and_destroy_model(&model);
      return false;
    }
    return true;
 ...
 private:
  bool parse_header() {
    ...
    while (model_source.read_next(buffer) && buffer != "SV") {
      if (buffer == "svm_type") {
        if (!model_source.read_next(buffer)) {
          fprintf(stderr, "Failed to read svm_type.\n");
          return false;
        }
        param.svm_type = -1;
        ...
        if (param.svm_type == -1) {
          fprintf(stderr, "Unknown svm_type\n");
          return false;
        }
      } else if (buffer == "kernel_type") {
        param.kernel_type = -1;
        if (!model_source.read_next(buffer)) {
          fprintf(stderr, "Failed to read kernel_type.\n");
          return false;
        }
        ...
        if (param.kernel_type == -1) {
          fprintf(stderr, "Unknown kernel_type\n");
          return false;
        }
      } else if (buffer == "degree") {
        if (!model_source.get(param.degree)) {
          fprintf(stderr, "Failed to read degree.\n");
          return false;
        }
      } else if (buffer == "gamma") {
        if (!model_source.get(param.gamma)) {
          fprintf(stderr, "Failed to read gamma.\n");
          return false;
        }
      } else if (buffer == "coef0") {
        if (!model_source.get(param.coef0)) {
          fprintf(stderr, "Failed to read coef0.\n");
          return false;
        }
      } else if (buffer == "nr_class") {
        if (!model_source.get(model->nr_class)) {
          fprintf(stderr, "Failed to read nr_class.\n");
          return false;
        }
        nr_class_permutations = model->nr_class * (model->nr_class - 1) / 2;
      } else if (buffer == "total_sv") {
        if (!model_source.get(model->l)) {
          fprintf(stderr, "Failed to read total_sv.\n");
          return false;
        }
      } else if (buffer == "rho") {
        model->rho = Malloc(double, nr_class_permutations);
        if (!model_source.get_array(model->rho, nr_class_permutations)) {
          fprintf(stderr, "Failed to read rho.\n");
          return false;
        }
      } else if (buffer == "label") {
        model->label = Malloc(int, model->nr_class);
        if (!model_source.get_array(model->label, model->nr_class)) {
          fprintf(stderr, "Failed to read label.\n");
          return false;
        }
      } else if (buffer == "probA") {
        model->probA = Malloc(double, nr_class_permutations);
        if (!model_source.get_array(model->probA, nr_class_permutations)) {
          fprintf(stderr, "Failed to read probA.\n");
          return false;
        }
      } else if (buffer == "probB") {
        model->probB = Malloc(double, nr_class_permutations);
        if (!model_source.get_array(model->probB, nr_class_permutations)) {
          fprintf(stderr, "Failed to read probB.\n");
          return false;
        }
      } else if (buffer == "nr_sv") {
        model->nSV = Malloc(int, model->nr_class);
        if (!model_source.get_array(model->nSV, model->nr_class)) {
          fprintf(stderr, "Failed to read nr_sv.\n");
          return false;
        }
      } else {
  ...
  bool parse_support_vectors() {
    ...
    for (int i = 0; i < model->l; ++i) {
      if (!model_source.read_line(line_buffer)) {
        fprintf(stderr, "Failed to read support vectors.\n");
        return false;
      }
      ...
      for (int c = 0; c < model->nr_class - 1; ++c) {
        if (!(line >> model->sv_coef[c][i])) {
          fprintf(stderr, "Failed to parse sv coefficients.\n");
          return false;
        }
      }
      ...
      while (line.good() && line >> node_buffer.index) {
        if (!line.ignore(1, ':') || !(line >> node_buffer.value)) {
          fprintf(stderr, "Failed to parse SVs.\n");
          return false;
        }
    ...
    // support vectors will be stored within a single memory plane, that is
    // indexed into by a pointer-array

    // create memory plane that stores the support vectors
    svm_node* support_vectors = Malloc(svm_node, sv_buffer.size());
    memcpy(support_vectors, sv_buffer.data(),
           sizeof(svm_node) * sv_buffer.size());
...
svm_model* svm_parse_model_from_buffer(const char* model_buffer,
                                       unsigned int length) {
